<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>JennyOS 🌌</title>

<style>
*{
margin:0;padding:0;box-sizing:border-box;font-family:"Poppins",sans-serif;
cursor:none;
}

/* Fondo espacio */
body{
background:#000;
color:#fff;min-height:100vh;overflow-x:hidden;
display:flex;flex-direction:column;align-items:center;
}

/* Loading Screen */
#loader{
position:fixed;top:0;left:0;width:100%;height:100%;
background:black;display:flex;flex-direction:column;
justify-content:center;align-items:center;z-index:9999;
color:#b388ff;font-size:1.2rem;gap:10px;
letter-spacing:1px;
}
.load-dot{
width:8px;height:8px;border-radius:50%;
background:#c084fc;animation:blink 1s infinite alternate;
}
@keyframes blink{
0{opacity:0.2;}100%{opacity:1;}
}

/* Canvas estrellas */
#stars{
position:fixed;top:0;left:0;width:100%;height:100%;
z-index:-1;
}

/* Glow orb */
.glow{
position:fixed;width:600px;height:600px;
background:rgba(170,0,255,0.12);
filter:blur(160px);border-radius:50%;
top:-200px;right:-200px;z-index:-1;
}

/* Cursor glow */
.cursor{
position:fixed;
width:18px;height:18px;
border-radius:50%;
background:#c084fc;
box-shadow:0 0 18px #c084fc80;
pointer-events:none;
z-index:9999;
}

/* Typewriter */
header{margin-top:3rem;text-align:center;}
.typewriter{
font-size:2rem;font-weight:600;border-right:3px solid #fff;
white-space:nowrap;overflow:hidden;width:0;
animation:typing 3s steps(30,end) forwards,blink .8s infinite;
}
@keyframes typing{from{width:0;}to{width:100%;}}
@keyframes blink{50%{border-color:transparent;}}

/* Cards secciones */
section{
width:85%;max-width:900px;margin:2rem auto;
padding:2rem;background:rgba(255,255,255,0.05);
backdrop-filter:blur(12px);
border:1px solid rgba(255,255,255,0.1);
border-radius:20px;
transition:0.3s;
}
section:hover{
border-color:#b388ff;
box-shadow:0 0 25px #b388ff40;
}

/* Titulos */
h2{
font-size:1.8rem;margin-bottom:1rem;
background:linear-gradient(90deg,#cd7cff,#7afcff);
-webkit-background-clip:text;color:transparent;
}

/* Fotos */
main img{
max-width:230px;border-radius:20px;margin:1rem;
box-shadow:0 0 30px rgba(255,255,255,0.2);
transition:0.3s;
}
main img:hover{transform:scale(1.05);}

/* Badge chips */
.badge{
display:inline-block;margin:5px;padding:8px 14px;
background:#15002a;border:1px solid #9e4aff;
border-radius:12px;font-size:0.9rem;
}

/* Progress bars */
.level{height:10px;width:100%;background:#2a2a3e;border-radius:10px;}
.fill{
height:100%;background:linear-gradient(90deg,#a855f7,#22d3ee);
border-radius:10px;
}

/* Footer */
footer{margin:2rem 0;font-size:0.9rem;opacity:0.7;}
a{color:#c084fc;text-decoration:none;}
a:hover{text-decoration:underline;}
</style>
</head>

<body>

<div id="loader">
loading JennyOS…
<div style="display:flex;gap:8px;">
<div class="load-dot"></div>
<div class="load-dot"></div>
<div class="load-dot"></div>
</div>
</div>

<canvas id="stars"></canvas>
<div class="glow"></div>
<div class="cursor" id="cursor"></div>

<header><div id="typewriter" class="typewriter"></div></header>

<main>
<img src="foto2jpg.jpg">
<img src="fotoJenny.jpg">
</main>

<section>
<h2>🌸 About me</h2>
<ul>
<li>Future AI researcher vibes</li>
<li>Chica tech polyglot</li>
<li>Runner girl</li>
<li>Amo los perritos</li>
</ul>
</section>

<section>
<h2>⚙️ Tech Stack</h2>
<div>
<span class="badge">Python</span>
<span class="badge">HTML</span>
<span class="badge">CSS</span>
<span class="badge">JavaScript</span>
<span class="badge">GitHub</span>
</div>
<p>Python</p><div class="level"><div class="fill" style="width:70%"></div></div>
<p>Web Dev</p><div class="level"><div class="fill" style="width:55%"></div></div>
<p>AI Learning</p><div class="level"><div class="fill" style="width:30%"></div></div>
</section>

<section>
<h2>🕒 Timeline</h2>
<ul>
<li>2023: Primer puesto</li>
<li>2024: Becas tech</li>
<li>2025: Web personal + CS50 + hackathon</li>
</ul>
</section>

<section>
<h2>🚀 Projects</h2>
<ul>
<li>Web personal</li>
<li>App sanitaria Figma</li>
<li>Producto vegano mockup</li>
<li>CS50 journey</li>
</ul>
</section>

<section>
<h2>🌈 Random Facts</h2>
<ul>
<li>Aprendo idiomas por diversión</li>
<li>Juego ajedrez</li>
<li>Me gusta conectar con gente creativa</li>
</ul>
</section>

<section style="text-align:center;">
<h2>📬 Contact</h2>
<a href="mailto:jennycabrerapalomino1@gmail.com">Email me 💫</a><br>
<a href="https://github.com/" target="_blank">GitHub</a>
</section>

<footer>made with stardust by Jenny © 2025</footer>

<script>
/* Typewriter */
const frases=[
"✨ Hola, soy Jenny",
"🌐 Hello, I am Jenny",
"🇯🇵 こんにちは、ジェニーです",
"🇩🇪 Hallo, ich bin Jenny",
"🌎 Ñuqaqa Jenny kani",
"👾 Aprendiendo para trabajar en IA",
"⚡ Building my future"
];
let i=0;
const t=document.getElementById("typewriter");
function type(){
t.textContent="";t.style.width="0";
let j=0;const int=setInterval(()=>{
if(j<frases[i].length){t.textContent+=frases[i][j];j++;}
else{clearInterval(int);setTimeout(del,1500);}
},120);}
function del(){
setTimeout(()=>{i=(i+1)%frases.length;type();document.title=frases[i];},2000);
}
type();

/* Cursor */
const cur=document.getElementById("cursor");
document.addEventListener("mousemove",e=>{
cur.style.left=e.pageX+"px";
cur.style.top=e.pageY+"px";
});

/* Stars */
const c=document.getElementById("stars");
const ctx=c.getContext("2d");
let w,h;
function resize(){w=c.width=window.innerWidth;h=c.height=window.innerHeight;}
resize();window.onresize=resize;
const stars=Array(200).fill().map(()=>({
x:Math.random()*w,y:Math.random()*h,z:Math.random()*3
}));
function draw(){
ctx.fillStyle="#000";ctx.fillRect(0,0,w,h);
ctx.fillStyle="#fff";
for(const s of stars){
const size=s.z*1.2;
ctx.fillRect(s.x,s.y,size,size);
s.x-=s.z; if(s.x<0){s.x=w;}
}
requestAnimationFrame(draw);}
draw();

/* Loader */
setTimeout(()=>document.getElementById("loader").style.display="none",1800);
</script>

</body>
</html>

